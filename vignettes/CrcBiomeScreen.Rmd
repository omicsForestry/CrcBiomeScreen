...
data(toydata, package = "CrcBiomeScreen")
...
data(simple_toydata, package = "CrcBiomeScreen")
colnames(simple_toydata)
# Create the CrcBiomeScreenObject for simple toy data
CrcBiomeScreenObject <- CreateCrcBiomeScreenObject(
  AbsoluteAbundance = simple_toydata[,-c(1:2,646:647)],
  TaxaData = colnames(simple_toydata[,-c(1:2,646:647)]),
  SampleData = simple_toydata[,c(1:2,646:647)]
)
...
You don't have to be limited to just the genus level. Use the `KeepTaxonomicLevel()` function to select the rank you need, whether it's **family**, **genus**, or **species**. The function will automatically group taxa at the specified level. For traceability, the output will always contain an `OriginalTaxa` column.
...
predictions <- PredictCrcBiomeScreen(
    CrcBiomeScreenObject, 

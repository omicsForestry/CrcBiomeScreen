...
-   **Integrated preprocessing**: automated taxonomic splitting, filtering, and normalization (TSS and GMPR);
-   **Modeling interface**: built-in machine learning modules for Random Forest and XGBoost classification with class weighting for imbalanced data;
-   **Evaluation framework**: functions for cross-validation, ROC/F1-based performance metrics, and external validation;
-   **Quality control utilities**: MDS-based sample outlier detection and contamination-aware filtering.
...
In general, the package expects microbiome abundance data to be provided in a **sample-by-feature format**, where each row represents a sample and each column represents a microbial feature, such as a genus, species, ASV, or OTU. The corresponding sample metadata should also contain one row per sample, with sample identifiers matching those in the abundance table. At minimum, the metadata should include a sample identifier column and a phenotype or condition column that defines the classification task.

The first example below uses the `Thomas_2018_RelativeAbundance` dataset and will be used for the subsequent steps in this vignette. This represents a typical use case with a more complex microbiome abundance table and associated sample metadata. The second example uses `NHSBCSP_screeningData` and is included to show an alternative input format for simpler datasets, demonstrating that the function can handle different data structures.

-   Expected abundance table format:

-   rows = samples

-   columns = microbial features

-   Expected sample metadata format:

-   rows = samples

-   columns = sample-level variables, such as sampleID and study_condition
...
CrcBiomeScreenObject_NHSBCSP_screeningData <- CreateCrcBiomeScreenObject(
  AbsoluteAbundance = t(NHSBCSP_screeningData[,-c(1:2,646:647)]),
...
## \> **Move to:** [Full-scale modeling workflow using real cohort data for reference](#%20Full-scale%20modeling%20workflow%20using%20real%20cohort%20data%20for%20reference)

# Runnable toy workflow setup

-   More explaination and details could check the Full-scale modeling workflow using real cohort data for reference section below.

## Setup the toy dataset
...
# -------------------------
# 3. Perform quality control using cmdscale
# -------------------------
...
str(validated_obj@PredictResult)
```

## Full screening workflow in one step
...
## Get the the raw prediction scores for each sample
...
# Full-scale modeling workflow using real cohort data for reference
...
`CrcBiomeScreen` supports two machine learning algorithms: **Random Forest** and **XGBoost**. Both models can be trained with class balancing to handle imbalanced datasets. For Random Forest models, the `mtry` tuning grid is automatically adjusted according to the number of available features in the training data. This prevents invalid parameter settings when working with small datasets or toy examples where the number of microbial features is lower than the default `mtry` values. For XGBoost models, a predefined tuning grid is used to search across key hyperparameters, including the number of boosting iterations, tree depth, learning rate, column subsampling, and row subsampling.
...
\*Note: The examples provided in this section are for illustration purposes only and are not intended to be run as-is.\*
...
# ValidationData_filtered_qc - Refer to the  Streamlined Screening Workflow

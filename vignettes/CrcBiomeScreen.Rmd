...
Full model training is disabled during vignette build because it exceeds SPB time limits.
All other steps are fully executable to illustrate real package behavior.
...
```
Model training is computationally intensive (cross-validation, tuning, ensemble models) and may exceed build time limits on the Bioconductor single package builder (SPB).
Therefore the vignette shows the full code but does not execute it.
Users can run the complete workflow on their own machines with adequate resources.

## Model training

Training machine-learning models for microbiome classification often requires repeated cross-validation, hyperparameter search, and ensemble methods.
These steps are computationally demanding and cannot be executed within the 15-minute limit of the Bioconductor build system.

For this reason, the full training code is provided for users, but is not executed when building the vignette.
Users may run the pipeline locally to reproduce the complete analysis.
...
head(getPredictResult(ValidationData_filtered_qc)$XGBoost)
predictions <- getPredictResult(ValidationData_filtered_qc)$XGBoost
```

...
#'
#' @examples
#' # Assuming you have a toy data object and a taxa table like this:
#' # CrcBiomeScreenObject <- CreateCrcBiomeScreenObject(...)
#' # my_taxa_table <- data.frame(
#' #   ASV_ID = c("ASV1", "ASV2", "ASV3"),
#' #   Taxonomy = c("k__Bacteria;p__Firmicutes;g__GenusA",
#' #                "k__Bacteria;p__Bacteroidetes;g__GenusB",
#' #                "k__Bacteria;p__Firmicutes;g__GenusC")
#' # )
...
LoadTaxaTable <- function(CrcBiomeScreenObject, taxa_table, id_column, taxa_column) {
  ...
  if (!id_column %in% colnames(taxa_table)) {
    stop(paste0("The specified 'id_column' ('", id_column, "') was not found in 'taxa_table'."))
  }
  if (!taxa_column %in% colnames(taxa_table)) {
    stop(paste0("The specified 'taxa_column' ('", taxa_column, "') was not found in 'taxa_table'."))
  }

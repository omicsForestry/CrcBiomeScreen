qcByCmdscale <- function(CrcBiomeScreenObject,
    ...
    dist_data <- vegan::vegdist(study_data, method = "bray")
...
# Output file   
    ...
    geom_point(data = subset(plot_df, IsOutlier == TRUE), 
                aes(x = Dim1, y = Dim2), 
                color = "red", size = 2, shape = 1, stroke = 1.2) +  # 离群点为红色
    geom_text(aes(label = SampleID, color = IsOutlier), size = 3, vjust = -0.6) +
    scale_color_manual(values = c(`FALSE` = "black", `TRUE` = "red")) +
    labs(title = paste0("cmdscale_", TaskName, " (", normalize_method, ")"),
        x = "PCoA 1", y = "PCoA 2") +
    theme_minimal() +
